%% ================== CONFIG ==================
IN_DIR  = 'C:\Users\crist\Desktop\Doctorado\datos_cuencas\valdivia';
OUT_DIR = 'C:\Users\crist\Desktop\Doctorado\datos_cuencas\valdivia\formateados';

if ~exist(OUT_DIR, 'dir'); mkdir(OUT_DIR); end
%% ============================================

clc; clearvars -except IN_DIR OUT_DIR;

inFiles = [dir(fullfile(IN_DIR, '*.xls')); dir(fullfile(IN_DIR, '*.xlsx'))];

if isempty(inFiles)
    fprintf('No se encontraron archivos .xls/.xlsx en: %s\n', IN_DIR);
    return;
end

for f = 1:numel(inFiles)

    IN_XLS = fullfile(inFiles(f).folder, inFiles(f).name);
    [~, baseName, ~] = fileparts(inFiles(f).name);
    OUT_CSV = fullfile(OUT_DIR, [baseName '_format.csv']);
    ...
    % Resumen rápido
    fprintf('Listo. Archivo %d/%d procesado. Registros exportados: %d\n', f, numel(inFiles), height(T));
    fprintf('Entrada: %s\n', IN_XLS);
    fprintf('Salida: %s\n\n', OUT_CSV);
end
